#!/usr/bin/env python3
"""
2022년 히스토리컬 데이터 수집 + Gemini 임베딩 + Supabase 벡터 저장

수집 항목:
  1) Upbit BTC/KRW 4시간봉 (2021-12-15 ~ 2022-12-31)
...
출력: data/historical_2022/ 디렉토리에 JSON 파일 + Supabase DB
"""
...
# ── 설정 ──────────────────────────────────────────────
DATA_START = datetime(2021, 12, 15, 0, 0, tzinfo=KST)  # RSI/SMA 선행 데이터
DATA_END = datetime(2022, 12, 31, 23, 59, tzinfo=KST)
SIM_START = datetime(2022, 1, 1, 0, 0, tzinfo=KST)      # 실제 시뮬 시작

OUTPUT_DIR = PROJECT_DIR / "data" / "historical_2022"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

random.seed(2022_01)  # 재현성
...
def collect_upbit_candles() -> list[dict]:
    """2021-12-15 ~ 2022-12-31 전체 4시간봉 수집."""
    all_candles = []
    cursor = DATA_END + timedelta(hours=4)

    print("[1/7] Upbit 4시간봉 캔들 수집 중 (2022년)...")
    batch_count = 0
    while True:
...
# =====================================================
#  Phase 2: Yahoo Finance 매크로 일봉 (2022년 실제 데이터)
# =====================================================
...
def collect_yahoo_macro() -> dict[str, list[dict]]:
    """2022년 전체 Yahoo Finance 매크로 일봉 수집."""
    print("[2/7] Yahoo Finance 매크로 수집 중 (2022년 실제 데이터)...")
    period1 = int(datetime(2021, 12, 1, tzinfo=UTC).timestamp())
    period2 = int(datetime(2023, 1, 5, tzinfo=UTC).timestamp())
...
def fetch_historical_fgi() -> dict[str, dict]:
    ...
    # 2022년 + 2021-12 필터
    fgi_2022 = {k: v for k, v in fgi_map.items() if k.startswith("2022") or k.startswith("2021-12")}
    print(f"  -> {len(fgi_2022)}일 FGI 수집 완료")
    return fgi_2022
...
def main():
    ...
    if DB_ENABLED and all_embeddings:
        ...
        for i, dp in enumerate(all_data_points):
            ...
            cycle_id = f"hist_2022_{dp['datetime']}"
            analysis_json = {
                "year": 2022,
