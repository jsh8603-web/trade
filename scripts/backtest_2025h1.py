#!/usr/bin/env python3
"""
2025년 상반기 백테스트 시뮬레이션 (1/1 ~ 6/30)

181일 × 6회/일 = ~1,086개 시점. 대규모 히스토리컬 데이터 기반.
Upbit 4시간봉 + FGI + 외부데이터(뉴스/고래/매크로/SNS) 시뮬레이션
→ 에이전트 매매 → Gemini 3072d 임베딩 → Supabase RAG DB 기록
"""
...
# ── 설정 ──────────────────────────────────────────────
SIM_START = datetime(2025, 1, 1, 0, 0, tzinfo=KST)
SIM_END = datetime(2025, 7, 1, 0, 0, tzinfo=KST)  # 6/30 24:00 = 7/1 00:00
INTERVAL_HOURS = 4
INITIAL_KRW = 5_000_000
MAX_TRADE_AMOUNT = 100_000
EMBEDDING_MODEL = "models/gemini-embedding-001"
BATCH_SIZE = 10  # 대규모이므로 배치 크기 증가
BACKTEST_ID = "bt_2025h1"
SOURCE_TAG = "backtest_2025h1"
...
def collect_all_candles() -> list[dict]:
    ...
    all_candles = []
    cursor = datetime(2025, 7, 1, 4, 0, tzinfo=KST)  # 약간 여유
    target_start = datetime(2024, 12, 15, 0, 0, tzinfo=KST)  # 지표 계산용 선행

    print("[1/6] Upbit 4시간봉 캔들 수집 중 (6개월+선행)...")
    while True:
...
def fetch_historical_fgi() -> dict[str, dict]:
    """Alternative.me에서 2025년 상반기 FGI 일별 수집."""
    print("[2/6] 히스토리컬 FGI 수집 중 (6개월)...")
    url = "https://api.alternative.me/fng/"
    # 2025-01-01 ~ 2025-06-30 = 181일 + 여유
...
def generate_embeddings(texts: list[str]) -> list[list[float]]:
    """Gemini embedding-001로 배치 임베딩 생성 (대규모 대응)."""
    embeddings = []
    total = len(texts)
    for i, text in enumerate(texts):
        for retry in range(3):
            try:
                ...
                if retry < 2:
                    wait = (retry + 1) * 5
                    print(f"  ⚠ 임베딩 #{i} 재시도({retry+1}/3): {e}, {wait}s 대기")
                    time.sleep(wait)
                else:
                    print(f"  ✗ 임베딩 #{i} 최종 실패: {e}")
                    embeddings.append(None)
        time.sleep(0.25)  # rate limit
        # 진행률
        if (i + 1) % 50 == 0:
            pct = (i + 1) / total * 100
            print(f"  → 임베딩 {i+1}/{total} ({pct:.0f}%)")
    return embeddings
    ...
    for i, (result, emb, emb_text) in enumerate(zip(results, embeddings, embedding_texts)):
        try:
            ...
            decision_row = {
                ...
                "source": "backtest_2025h1",
                "cycle_id": f"bt_2025h1_{i:04d}",
...
def main():
    ...
    # 결과 로컬 저장
    output_path = PROJECT_DIR / "data" / "backtest_2025h1_results.json"
