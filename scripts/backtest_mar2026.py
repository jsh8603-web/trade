#!/usr/bin/env python3
"""
2026년 3월 백테스트 시뮬레이션 — 히스토리컬 데이터 기반 매매 + RAG 임베딩 기록

1. Upbit 4시간봉 캔들 수집 (2026-02-15 ~ 2026-03-21, 지표 계산용 선행 데이터 포함)
...
# ── 설정 ──────────────────────────────────────────────
SIM_START = datetime(2026, 3, 1, 0, 0, tzinfo=KST)
SIM_END = datetime(2026, 3, 22, 0, 0, tzinfo=KST)  # 3/21 24:00 = 3/22 00:00
INTERVAL_HOURS = 4
INITIAL_KRW = 5_000_000  # 시뮬레이션 시작 자금 500만원
MAX_TRADE_AMOUNT = 100_000
EMBEDDING_MODEL = "models/gemini-embedding-001"
BATCH_SIZE = 5  # 임베딩 배치 크기 (rate limit 대응)
BACKTEST_ID = "bt_mar2026"
...
def collect_all_candles() -> list[dict]:
    ...
    all_candles = []
    cursor = datetime(2026, 3, 22, 4, 0, tzinfo=KST)  # 약간 여유
    target_start = datetime(2026, 2, 15, 0, 0, tzinfo=KST)
    ...
    for i, (result, emb, emb_text) in enumerate(zip(results, embeddings, embedding_texts)):
        try:
            ...
            decision_row = {
                ...
                "source": "backtest_mar2026",
                "cycle_id": f"bt_mar2026_{i:03d}",
...
def main():
    ...
    # 결과 로컬 저장
    output_path = PROJECT_DIR / "data" / "backtest_mar2026_results.json"
