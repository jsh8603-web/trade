...
> 인계: [handoff-ic8-fx-wire-20260602.md](./handoff-ic8-fx-wire-20260602.md) — IC8(A→B 전환)·IC4 graduation·독립 audit 완료. ★fx 정밀화 ✅완료(2026-06-02): Λfx 0.09→0.15(자문 2모델 B 만장일치 + falsification F1/F2/F4 실측, IC8 섹션). 안전판=fx_hedge=full(IC10 복원). 잔여=실 FRED Λ 경로 활성화(현 항상 eye fallback).
> ★최신 인계: [handoff-axis-AF-20260602.md](./handoff-axis-AF-20260602.md) — factor 축 A~F 자문수렴(Phase X)+E·F감사완료. **A(term spread+breakeven factor 추가)=다음착수**(VIF측정 `.tmp-A-vif.py` 실행만). 미답=지표(indicator) 누락 전수조사(거시+전자산군, 축과 별개)→subagent.
> ★★최신 인계: [handoff-e2e-wire-20260602.md](./handoff-e2e-wire-20260602.md) — e2e 통합개통 Y1~Y4b + **Y5 factor codify ✅**(v1.36.0: rate→real+breakeven, MOVE/slope DROP=joint mv β→0 VIX흡수 이중계상, 자문 2모델 만장일치, FACTORS 6→7, off byte-identical, golden 0.288/0.272, ledger §6-Y5). 자율주행ON(flag+long-mode, Phase V 게이트). ★남=fetch캐시(255s→수초)·jm시계열·최종 e2e테스트·★weights관찰(corr_prior 발동해도 BL prior근처=cov_override 경로 점검).
...
- [x] 시뮬: self-test 1~9 PASS(B·Λ·Bᵀ 6×6 PSD, fx denomination 원값 보존·hedge=full→0). factor_returns 6×6 PSD. 회귀 passed, off byte-identical(_ic_corr_prior None).
- [x] ★A→B 전환(2026-06-02, 자문 2모델+코드검증+독립 audit 수렴): 외부 자문 gemini/claude **A(+0.30 정규화) 만장일치 기각**(denomination=추정 β 아닌 정확히 1.0 결정론값, fudge 왜곡). ★코드검증(무비판 차단)=A/B는 corr_prior 에서 **fx_β²·Λfx 곱 등가**(magnitude FREEZE cov2corr) → 차이는 실 Λ 경로뿐(fx_β=1.0이어야 실 환율변동성 1:1). Claude "gold fx와 직교라 decoupling 안깸"=**코드 반증**(gold×cyclical 0.0637→0.1177 동조↑). → **B 적용**: SEED fx_β 전부 +1.0(절대 denomination, gold 포함 full) + `_static_factor_lambda` eye fallback **Λfx=0.09 ≈ (USDKRW/자산 변동성)² ≈ 0.3²**. golden us×commodity **0.2642 불변**(USD 곱 1.0²·0.09 = A 0.30²·1.0 등가)·us×gold 0.1298→**0.1994**(gold full=KRW 환노출 현실)·fx_hedge=full→**IC10 0.0832 정확 복원**. 회귀 120·off byte-identical·PSD.
- [x] ★독립 audit(general-purpose ab639df27606, 2026-06-02): **IC8 조건부통과** — eye-fallback(FRED 미가용 기본경로)서 fx 가 gold×cyclical 공분산 **70.5% 지배**(+0.128 중 +0.090) = main "dollar/vol 열 보존" 표현 **부정확** 정정(fx 가 모든 USD sleeve 에 +0.09 floor 까는 **새 지배 공통인자**, KRW 묶음 공통 환노출 현실이나 지배도가 Λfx 단일상수 민감). 배분 비정상 아님(corr<0.3), fx_hedge=full 복원. Λfx=0.09 근거 박제 완료(0.3² calibration). **IC4 통과** — 5-AND 재사용·자가승격 가드·idempotent·off byte-identical·go-live 격리(호출처 0) 코드+test 확인. ★go-live wire 시 사이클 호출자가 self_owner 주입해야 자가승격 방어 작동(주의점). 잔여 보완=Λfx 실 USDKRW 변동성 측정 박제(현 0.3² 추정)·실 FRED Λ 경로 fx 기여 확인.
- [x] ★Λfx 정밀화 0.09→0.15 ✅ (2026-06-02, 자문 2모델 + 자체 falsification 실측 — 위 audit 잔여 보완 해결):
  - **측정** (FRED DEXKOUS/VIXCLS/DTWEXBGS + yfinance, 2006~2026 n≈5037): σ_fx(USDKRW Δlog)=0.0070, σ_asset median=0.0122 → σ_fx/σ_asset=0.575. SEED β=표준화 회귀계수라 표준화 정합값 Λfx=(σ_fx/σ_asset)²=**0.33**(naive). 이전 0.09="0.3² 어림"=근거 없는 추정 확인.
  - **자문**(/gemini-web + /claude-web 병렬): **둘 다 B(Λfx 0.15~0.20) 만장일치 권고, A(0.33 박제) 기각**. 근거=0.33이면 fx 가 cross-corr 76~90% 지배=rank-1 퇴화("전 USD자산 한 덩어리"=allocator 재료 소거) + magnitude FREEZE 가 shrink 명령(0.33 점추정 박제가 위반). gold decoupling(IC10)도 90% 가림. Claude 통찰: A=C(sleeve별)의 median-collapse 특수해, sleeve편차(reit 0.38~defensive 0.78)는 noise 아닌 구조신호.
  - **자체 falsification 실측**(decision-quality 본인 시뮬 의무, 자문 F1/F2/F4): **F1 직교성** corr(USDKRW,dollar TWI)=+0.516 강(fx⊥dollar 거짓=이중계상 위험), R²(USDKRW~VIX+TWI)=0.268 → 직교할인 Λfx=0.33·(1−R²)=**0.242 상한**. **F4 무조건부 target**(prior 목적=realized 재현): KRW-표시 realized gold×equity corr=**+0.17~0.20**, implied 0.33→+0.288(과대)/0.15→+0.20(일치)/0.09→+0.166(미달). **F2 regime**: calm σ비 0.653(Λfx=0.426)로 오히려 높음(자문 de-bias 가설 기각, 보수값 채택).
  - **확정 Λfx=0.15**: F4 target(0.17~0.20) 재현 + F1 직교상한(0.242) 하회 + 자문 band(0.15~0.20) 하단. fx 지배 ~80%(0.33의 90% 대비 완화, rate/vol/dollar 차별화 + gold decoupling 일부 보존).
  - **코드**: `regime_to_weights._static_factor_lambda` Λfx 0.09→0.15 + 근거 주석 / `factor_betas_seed` 주석 갱신 / golden `test_sleeve_belief_cov` us_stock×commodity 0.2642→**0.2951**·us_stock×gold 0.1994→**0.249**(fx_hedge=full→0.0832/0.2026 IC10 정확 복원 불변). self-test 9 PASS·golden 9 passed·전체 회귀 4435 passed/36 failed(전부 기존 baseline, IC8 무관)·off byte-identical.
  - **별도 발견**(잔여): 실 FRED Λ 경로(`fetch_factor_cov` 6factor)는 rate/vol/fx first-release vintage 부재로 fetch 실패(`get_series_first_release` None) + `_default_fred_source` 가 키 없이 adapter 생성 → shape≠(6,6) → **항상 eye fallback**. 즉 Λfx 상수가 fx 기여 정하는 유일 손잡이. 실 Λ 경로 활성화=별도 과제(키 주입 경로 + first-release 우회).
...
- ⊘ **(b) SLEEVE_AGG 매핑 확장 — 폐기 확정(사용자 2026-06-02: "슬리브 추가는 안 하기로 했다")**: 신규 배분 sleeve(reit/eq_intl/xle) 추가 안 함. 현 배분 SLEEVES(us_stock/kr_stock/commodity/gold/bond/cash/coin) 고정. ★defensive 는 SLEEVE_AGG us_stock←cyclical+defensive 흡수로 이미 반영(measured vol −0.63 roll-up). reit/eq_intl/xle 은 SEED_CELLS factor β 는 보유(corr_prior 분해 참여 가능)하나 SLEEVE_AGG roll-up 미매핑=배분 corr_prior 무기여 유지 — 정직 라벨. → 이들 study(verdict 격상·L축 재검)는 배분 런타임 영향 없음, study 정합성 보강 의미만(우선순위 하향).
...
- **관문**: ✅**(a)(c) 완료**(measured 배분 레이어 박제 + measured-vs-yaml 검증 충돌 0, 회귀 35<37) / ⊘(b)=폐기 확정(사용자 2026-06-02 "슬리브 추가 안 함", reit/eq_intl/xle 배분 미매핑 고정) / ⛔(d)=judge 소비 go-live 경계. magnitude=측정(shadow 폐기). crypto=small-n UNKNOWN(yaml 5건 정정 후 IC9 복귀).
...
## Phase X — factor 상관 "축" 확장 검토 (A~F, 2026-06-02 외부조사+자문 2모델 수렴)
> **배경**: 논문(CRR 1986·Costa-Kwon 2020·FGL Lee-Seregina 2020·Kritzman turbulence 2010·Forbes-Rigobon 2002) + GitHub 2조사(subagent aa25c9 논문 / af5cb94 OSS) → **우리 축은 표준 강화판 확인**(factor-implied Σ=BΛBᵀ + RegimeGlasso belief-mix = Costa-Kwon 동형, 우리가 nonparanormal+EBIC+EB shrink로 더 정교). 누락 축 후보 A~F 도출 → 자문 2모델(gemini-web/claude-web 2026-06-02) 수렴.
> ★**우리 특화 보존(버리지 말 것)**: fx denomination factor(KRW, OSS 미발견 특화) / factor-implied(n<30 멀티에셋엔 DCC보다 안정, 추정폭발 회피) / Costa-Kwon 강화판. SEED β 수치도 학술 뒷받침(gold-dollar -0.317=multivariate attenuation 정상, stock-vol -0.687 정합, gold-vol≈0 reject=safe-haven 시변 정합).
> **자문 수렴 우선순위 = E→A→B / C·D·F 강등**(양모델 A+E 최우선 동의, claude은 E를 A앞에=feed-forward). 도입 비용 전제: factor 축 추가=VIF 재측정+small-n rigor(p/CI/Bonferroni)+opt-in off byte-identical.

- [ ] **E (1순위 — 감사·정량화, 가장 쌈, A에 feed-forward)**: `core/study/factor_cov_estimate.py` stress floor `ρ←max(ρ_EWMA,ρ_stress)` 데이터흐름 검증.
  - **할 것**: (1) stress기에 ρ_EWMA가 이미 팽창→floor moot인지 / ρ_stress와 stacking인지 grep+측정. (2) ρ_stress가 raw 위기상관(Forbes-Rigobon artifact 상속)인지 FR-adjusted(변동성 통제)인지 확인. (3) FR-adjusted stress 상관 산출→raw와 비교→floor를 FR-adjusted+선택 margin으로 재설정=conservatism을 legible·bounded하게.
  - ★**FR도 ground truth 아님**(claude): 진짜 contagion(관계 자체 변화) 시 FR이 반대로 over-correct(진짜 동조를 artifact로 깎음). 목표=over-correction "제거"가 아니라 보수성을 정량·유계화. 신규 A factor가 이 clamp 상속하므로 E 먼저.
  - ✅ **[감사 완료 2026-06-02]**: stress floor `stress_corr` 인자 = **런타임 미주입**(fetch_factor_cov static Λ·gate-Λ 둘 다 stress_corr 없이 estimate_factor_cov 호출 / self-test factor_cov_estimate.py:200·206 에서만 주입) → **stress floor = dead path**. Forbes-Rigobon over-correction **현 미발현**. RegimeGlasso regime별 실측 corr 고vol bias 여지는 nonparanormal rank-transform 이 정규성 가정 풀어 일부 완화. → **조치 불필요. 향후 stress floor 활성화(gate-Λ wire) 시 ρ_stress=FR-adjusted 적용** 권고만(미래 가드).
- [x] **F (grep 완료 → 강등 확정)**: `_ic_corr_prior`(factor-implied BΛBᵀ→cov2corr) → `cp` → `RegimeGlasso(corr_prior=cp).fit()` (regime_to_weights.py:307) → `eb_shrink(corr_glasso, corr_prior)=lam·corr_prior+(1−lam)·corr_glasso` (conditional_correlation.py:213). **factor Σ가 RegimeGlasso input prior 로 흘러감 확인** → idio sparse(FGL off-diag) 추가 시 RegimeGlasso sparse precision 이 allocation 서 동일 co-move 복원=**이중계상 → 강등 확정**. 현 diagonal freeze=systematic/residual 깨끗분리+잔차의존 소유권 RegimeGlasso 이관=good design 입증.
- [ ] **A (2순위 — E 반영 후)**: term spread(T10Y2Y slope) + inflation breakeven(T5YIE)를 FACTORS 축에 추가. CRR 정전, gold/commodity inflation driver. ★현황: breakeven_5y·T10Y2Y는 `regime_classifier`에 evidence로 이미 사용, **factor 공분산 축엔 없음**(rate=DGS10 level만).
  - ★**이중사용 아님**(양모델): regime=coarse conditioning(어느 Λ 쓸지), factor=fine structure(Σ 내용). 표준 conditional covariance modeling.
  - ★**진짜 risk=within-regime variance truncation**(claude): breakeven이 regime 정의(inflation_z)+factor 동시→그 regime 내부 factor variation 절단→조건부 분산 좁게 noisy. **해소=regime엔 level(inflation_z 사분면)/factor엔 change(breakeven Δ·잔차)**.
  - ★**직교화 필수**(VIF): nominal 10Y=real+breakeven 공유 → {nominal level, breakeven} collinear. **real-rate(DFII10)+breakeven+slope 3축**이 더 깨끗(or Gram-Schmidt 잔차/PCA level-slope-curvature). kill=VIF≥5 or 기존 1.14 대비 급등 or regime-conditional Λ condition number 발산→철회.
  - ✅ **[VIF 실측 2026-06-02, `.tmp-A-vif.py` Δ공통표본 n=740(2023-06~, credit 절단)]**: 현행6 max=1.76(dollar) / 조합A(nominal+slope+breakeven) max=1.88, `corr(nominal,breakeven)=+0.496` / **조합B(real+breakeven+slope) max=1.82, 전 pairwise corr 낮음**. ★결정타 `corr(nominal,real)=+0.914`(nominal=real+breakeven 구조중복) → **조합B 채택 확정**(nominal 빼고 real 교체로 중복 제거, claude 자문 권고와 일치). 둘 다 VIF<5 통과지만 B가 직교 우수.
  - ⛔ **codify 전제(사용자 2026-06-02)**: A를 VIF만으로 박지 않음. **cross/regime 상관 실측(15축 근거) + opus 독립 audit(15축) verdict(adopted/candidate/reject) 확정 후** SEED β codify — 다른 cross factor와 동일 4관문 절차. series 공백 전수조사(subagent)와 묶어 진행: 발굴→cross/regime 측정(2차 실측, 15축+small-n rigor)→독립 audit(15축, self-certify 금지)→확정분만 codify(재측정 2회 회피).
    - ★**15축 = 12 study축(A~L) + 3 wire축(M~O)**, hard 코어=B·C·D·I·M·N·O. 카드=`study-research/_wire/15axis-summary.md`(자기완결, P1/P2 subagent 그대로 전달, 재작성 X). 실측·audit 둘 다 15축 적용(사용자 2026-06-02 명시). 논문/GitHub 주장은 "claim"으로 취급→우리 데이터 재실측(AUDIT-GUIDE §0 제1원리).
- [ ] **B (중기 — retarget)**: liquidity/funding factor. ★TED=LIBOR폐지(2022~) dead → **USD/KRW cross-currency basis**(KRW 투자자 funding 1차 경로, 기존 fx factor 보완) or SOFR-OIS. kill=stress sub-sample partial corr(VIX·HY OAS 통제 후) 비유의면 증분정보 0→강등.
- [ ] **C (보류 — 형태 제한)**: regime 다축화. ⛔**partition 형태 금지**(거시 4분면×vol 2=8 regime→cell별 obs 붕괴→RegimeGlasso Λ 폭발, n<30서 추정 자체 깨짐). turbulence(Kritzman Mahalanobis)는 **continuous-modulator만 허용**(belief-mix weight or shrinkage intensity 연속 dial, 현 JumpModel confidence layer 흡수=분할 안 함). kill=regime cell effective n<15면 즉시 연속형 회귀.
- [ ] **D (연구 — sandbox만)**: copula/tail-dependence. ⛔**production 강등**(n<30 joint-tail obs≈0~2=noise fit + copula가 tail 상관 magnitude 재형성→magnitude FREEZE invariant 충돌). sandbox 유일 용도=long single-pair history에 t-copula 적합해 "Gaussian이 얼마나 틀리는지" magnitude 측정→E floor 폭 근거로만. stress floor가 이미 하방동조 조잡 proxy(E 제대로 calibrate=D의 80% 효용을 5% 비용에).
- **관문**: E·F=기존 코드 감사(자율, 측정·grep) / A=factor 축 추가(자율, VIF+small-n+무회귀 게이트) / B·C·D=보류·연구. **착수 순서 E+F(병행 grep)→A**. decision-quality=본인 시뮬 의무(grep+측정 동반).

### Phase X-2 — series 공백 조사 + ROI 선별 (2026-06-02, 발굴 subagent afd5bb1)
> **1차 발굴**: ledger 3종 + Gemini 2R 조사. ★발견=자산군 내부 study series 이미 촘촘(adopted 71/candidate 54). 진짜 공백 2곳=(1)**factor 레이어 승격 누락**(FACTOR_SERIES 6개뿐, A real/breakeven/slope조차 미배선) (2)**KRW 고유 + 펀딩/유동성 축**.
> **ROI 선별**(사용자 2026-06-02 "설명도·ROI 높은 것만, 증명해도 가치 없는 건 빼자" + "월별=동적 아니면 빼"): 잣대=우리 배분 sleeve(us_stock/kr_stock/commodity/gold/bond/cash/coin)에 실제 영향 + 일별 동적.

- [ ] **2차 검증 대상 3축 확정**(전부 일별 동적):
  - **A factor** real(DFII10)+breakeven(T5YIE)+slope(T10Y2Y) — 조합B 확정(rate 명목 제외, real 교체. VIF max 1.82).
  - **MOVE**(국채 IV) — VIX(주식공포)와 **직교**(금리경로 불확실성 별 축), bond·gold 위험채널. ★source 검증 선행(FRED 없음→Yahoo `^MOVE`).
  - **FRA-OIS/SOFR-OIS**(펀딩 스트레스) — "위기 시 상관→1 수렴" 외생 동인, 전 sleeve stress 동조→JumpModel stress trigger. ★LIBOR→SOFR splice 선행.
- ⊘ **제외(증명해도 가치 없음/규칙 충돌)**: reit eREIT/mREIT·eq_intl EMBI/GFCy(배분 미매핑→무영향) / 외국인순매수(reflexive loop 불변식 충돌=학습입력 금지) / term premium·10Y-3M(slope 중복+regime에 yield curve 이미 有→slope 검증 후 incremental 시만) / copper-gold(growth regime 중복) / crypto NUPL/SOPR(MVRV adopted와 중복) / **Global PMI(월별=동적 지표 아님, 사용자 제외)**.
- **절차**: 2차 P1 실측(15축 근거+small-n rigor, ★FRED id 검증 먼저) → P2 독립 opus audit(15축, self-certify 금지) → 통과분만 codify(FACTOR_SERIES/SEED_CELLS 확장, opt-in off byte-identical, golden 재측정, 회귀).
- ✅ **P2 독립 opus audit 완료(2026-06-02, a8f746b2, raw 재계산 hard-fail 0)**: MOVE/real(bond·gold)/breakeven(commod) **adopted** · slope **candidate**(commod, bond는 real중복 L축) · funding **rejected_provisional**(IC9 부활). breakeven PARTIAL→adopted 격상(block-boot). MOVE transform=Δln. → cross-regime-ledger §6 박제 완료.

### ★Phase Y — e2e 통합 개통 (사용자 방향 2026-06-02: "조각 따로 보지 말고 한꺼번에 굴려라")
> **진단(사용자)**: 시스템이 opt-in facade라 조각마다 dormant(corr_prior=eye·judge 호출0·거시 로깅만) → study→regime→corr_prior→allocate 가 한꺼번에 안 굴러감. **"한꺼번에 안 굴러야 할 이유 없으면 전부 잇는다."**
> **개통 범위(잇는다)**: 거시지표 PIT fetch → regime_classifier → RegimeGlasso 학습 → corr_prior(opt-in on) → allocate(sleeve 비중%). **off=byte-identical 안전판 유지**(회귀 보호), on 경로가 실제로 한꺼번에 산출.
> **게이트(여기까지만)**: judge 실 사이징 소비 · execute_trade 실주문 = go-live 경계(사용자 게이트). 배분% 산출까지만 e2e, 실집행 비접촉.
- ✅ **진단 완료(ad848, 2026-06-02)**: 현 국면 STAGFLATION(growth_z −0.482/infl_z +1.09, conf 0.62) / 배분 cash 20.25% 최우위. ★단 **입력 절반 결손**으로 나온 잠정 판정. 끊긴 지점 5:
  - (Y1) **fred_adapter first_release 일별 6종 silent drop**: T5YIE/T10Y2Y/DFII10/DGS10/DGS2/BAA10Y = ALFRED get_series_first_release vintage 과다(3051) ValueError → breakeven/금리커브/실질금리 입력 결손(infl_z가 core CPI 단독). ★P2 audit가 이 시리즈 revision=0 입증→latest=first_release 동일, latest fallback PIT 안전.
  - (Y2) NFCI/STLFSI4 first_release 깨진 vintage(stale 1976/2004)
  - (Y3) JumpModel 미설치(jumpmodels) → stress/calm 메타신호 부재
  - (Y4) corr_prior dormant: sleeve_regime_ids substrate 부재 → belief 기록만, _ic_corr_prior 미발동(IC0-R)
  - (Y5) fetch silent skip(study §1.1 위반) → 경보 부재
- [x] **Y1 fred_adapter fix ✅**(2026-06-02): `_MARKET_PRICED_DAILY`(DFII10/T5YIE/T10Y2Y/DGS10/DGS2/BAA10Y) first_release→get_series(latest) 직행, vintage ValueError 회피. 검증: bundle에 breakeven 2.52/curve 0.47/real 2.06/credit 1.57 진입(fix 전 silent drop), as_of PIT mask 안전(2020-01-01→2019-12-31).
  - 회귀 확인: fred/regime/brain **134 passed**. macro_vintage 4 fail = ★test pollution(fix복원+단독 12p / fix없음+단독 12p → fix 유무 무관, 전체 -k 범위서만 fail = 타 테스트 FRED_API_KEY 환경누수) **fix 직교 확정**(E94 변수분리 검증).
- [x] **Y2 NFCI/STLFSI4 stale fix ✅**(2026-06-02): first_release 가 NFCI(1976)/STLFSI4(2004) stale 반환(silent 오염, 현 STAGFLATION nfci=−0.88이 1976값이었음) → `_FIRST_RELEASE_BROKEN`+`_LATEST_FALLBACK` latest 경로. NFCI −0.88(1976)→−0.51(2026) 정상화. ★백테스트 PIT vintage=Phase V TODO(NFCI revision mean 0.35 큼). 회귀 55 passed.
- [x] **Y3 jumpmodels 설치 ✅**(2026-06-02): jumpmodels 0.1.1 설치, import OK(`regime_classifier:282` from jumpmodels.jump import JumpModel). requirements.txt 등재. → jm_status unavailable→ok 전환(stress/calm 메타신호 활성, 실작동은 e2e 재산출서 확인).
- [x] **Y4 corr_prior 활성 ✅ connectivity 입증**(2026-06-02): coin_track_macro 가 substrate(fetch_sleeve_returns IC0 + build_sleeve_regime_ids IC0-R) **이미 배선**. 현 국면 subagent 는 allocate 직접호출로 우회→dormant 오인. e2e 검증(경량): belief+ids+returns 3-AND 충족 시 **belief_conditional_cov(Σ_eff regime-glasso) 발동 YES**, method=black_litterman_returns. 현 국면 재산출 STAGFLATION(growth_z −0.429/**infl_z 0.548**[Y1~2 breakeven 반영, 이전 1.09 core CPI 단독→해소]/conf 0.45/yc_spread 0.47).
  - ⚠️ **Y4b 후속(라이브 실효성, 자율범위)**: (a) `build_sleeve_regime_ids`=과거 전체 classify(FRED fetch 수백~수천)=라이브 부적합(20분+, regime_history:94 경고) → **배치 캐시/증분 필요**. 합성 ids 검증 통과하나 실 regime ids 배분 영향은 캐시 후. (b) jm_status=insufficient_data(JM 시계열 학습 경로 필요, 단발 classify 부족). (c) 합성 ids라 weights=BL prior 근처(실 ids 면 Σ_eff 배분 변경).
- [x] **Y5 factor codify ✅**(2026-06-02, v1.36.0): rate(DGS10 명목)→**real(DFII10) 교체** + **breakeven(T5YIE) 추가**, **MOVE/slope DROP**. ★핵심: P2 audit 헤드라인 β(MOVE us −0.213/slope commod +0.119)는 **univariate**였고, 통일 8-factor joint multivariate 재측정(batch-std-beta-9factor, n≈5000, MAX VIF 1.26) 시 MOVE/slope β→0 붕괴(VIX가 risk-off 분산 완전 흡수). factor 공분산 prior B·Λ·Bᵀ엔 joint β 정합 — univariate 박으면 VIX와 **이중계상(L축 위반)**. **외부자문 2모델(gemini/claude) 만장일치** Option A(drop). re-scope framing(audit=marginal confirmed / covariance-incremental rejected, 채택게이트 업그레이드). FACTORS 6→**7**(real/dollar/oil/credit/vol/breakeven/fx). self-test PASS(gold real −0.233=명목 −0.2155 재현·decoupling Δ0.02 보존·reject lock). corr_prior 7×7 PSD 발동·**off byte-identical(None)**·golden us×commodity 0.2951→**0.2880**/us×gold 0.249→**0.2721**. 회귀: 도메인 273 passed 0 fail·격리 63 passed(전체 38fail=test pollution+working-tree-clean+golden-meta, factor import 0=Y5 직교 확정). ledger §6-Y5 reconcile 박제. → cross-regime-ledger.md §6
  - ⚠️ **MOVE/slope 부활 트리거**(IC9 대상): bond sleeve corr_prior 편입(MOVE=채권 IV 전용) OR regime-conditional 분리(rate-stress vs equity-stress). 현 3 sleeve(equity/commod/gold)=VIX-dominated 라 full-sample drop.
- [x] **e2e 테스트 ✅**(2026-06-02): 진입점 `collect_market_state`(coin_track_macro) capstone — `tests/test_e2e_collect_market_state_r15_wire.py` 3 passed. **on** → substrate(returns+macro_view+sleeve_regime_ids 2국면) **allocate 실연결** + `belief_conditional_cov` 동적 Σ_eff 발동 + macro_weights 합=1. **off** → fetch 0회 + allocate 정적(None) 경로 + r15_belief 미산출(무회귀) + 정적 macro_weights. **graceful** → classify 예외(FRED 실패 모사) → 예외 전파 0, 정적 fallback. 실 FRED 29s = CI 부적합 → fetch_sleeve_returns·RegimeClassifier·RealFredAdapter 합성 patch + CoinTrack override 4종 격리. orchestrator.allocate 내부(golden/belief 동적성/PSD)=test_sleeve_belief_cov.py 분담(중복 회피). 신규 테스트=기존 코드 0변경(직교).
- [x] **★weights 관찰 ✅해소**(2026-06-02): mechanism live test — stance 비움 시 weights L1 Δ=0(BL **reverse-opt 항등식**: view 없으면 π=δΣw로 cov 상쇄=cov-invariant, 정상) / stance 포함 시 belief calm↔crisis 가 **L1 Δ=0.136** 이동(coin·commodity·kr·bond 반응). ★배선 LIVE 확정 — 이전 "BL prior 근처"는 버그 아니라 stance 약함/degenerate ids. corr_prior/Σ_eff(+Y5 factor 전체)는 **macro view(stance) 있을 때만** 배분 작용(의도된 BL 설계).
- [x] **fetch 캐시 ✅**(2026-06-02, v1.36.1): `fred_adapter.get_series` raw(pre-mask) series_id별 instance 캐시(`_raw_cache`), as_of mask는 매 호출 fresh slice(PIT 보존). build_sleeve_regime_ids **255s→29.2s(8.7x)**. 검증: PIT mask 보존(as_of=2020 14484<2024 15485<full 16086, lookahead 0)·재호출 2ms·macro_vintage 12 + fred/regime/brain 86 passed. 인스턴스 수명=단일 build/collect(매 collect 신규 adapter)→stale 0.
- [x] **(a) jm 시계열 경로 ✅**(2026-06-02, v1.36.3): `_jump_model_overlay` insufficient_data 근본원인 = `_build_jm_matrix` **빈도 불일치**(실측 진단 `.tmp-jm-diag.py`: X shape **(0,N)** 0행). 월별 yoy(매월 1일)·resample ME diff(월말)·breakeven 일별 yoy(diff_list 누락→일별) **세 빈도 혼합** → `DataFrame(cols).dropna()` inner-join 교집합 ≈ 0 → 단발/시계열 무관 *항상* insufficient. **fix**: 모든 시리즈 `ME.last()` 월말 통일 후 변환 + breakeven_5y → diff_names 편입 + 변환후 60mo 미만(hy_oas 35mo 등) 컬럼 제외. **효과 실측**: `_build_jm_matrix` X **(278,8)**, classify jm_status **insufficient_data→ok**, jm_state=calm, divergence 0.0, conf 0.606. 회귀 도메인 242 passed 0 fail + 신규 `tests/test_jm_matrix_freq_align.py` 3 passed(합성 혼합빈도 정렬·짧은시리즈 제외·graceful). `_is_high_freq` dead-code화(보존). ⚠️관찰(JM 무관): inflation_z 1.154(핸드오프 0.548 대비↑, _inflation_signal 미접촉=FRED CPI/breakeven 갱신 추정, 다음 세션 확인).
