...
- [x] **W3 섹터 regime틸트** ✅DONE(2026-06-08~09 검증): `decompose_weight(factor_z=)` regime-conditional 보간 배선(factor_z=None byte-identical), `_DEFENSIVE_BAND=(0.10,0.28)` full range 복원, cyclical cap·mega 고정, `_bt_real_rate_z` 호출처 주입. 회귀 PASS(ckpt-090900). EW 갈아엎기 안 함(시총비례 anchor 유지).
- [x] **W6 측정정정** ✅DONE: 선택alpha 분모=업종 전체 universe EW(run_multiasset:711,799,1073,1109 `ind_universe` 선택 전 종목) tautology 정정, ⑮거래비용(turnover×cost 한국STT0.25%/기타0.07%, :1132), ⑭survivorship phantom haircut ~1.2%p/yr 동반해석 라벨.
- [x] **W4 한국선택** ✅DONE(P8-A): eq_kr cheapness×quality(ROE) 트랩 게이트(`_kr_pit_roe`, semi/aitech cs_pbr_z·steel mom_12_1), ρ 살아있는 sleeve만 개별선택 나머지 ETF/EW. ★단 10년 OOS=산업내 selection 음수 확인(WHY-ROTATION-SELECTION 문서)→최종 ETF/EW+소액 정적 틸트로 확정(ledger §41-42).
- [x] **W1 종목비중** ✅DONE(2026-06-09 실측, `.p7-w1-invvol.py` walk-forward no-lookahead): **capped-EW 현행 유지 확정**. 15 sleeve(us 3 + kr 12) EW vs inverse-vol(trailing 60d vol lagged, 단일 cap 0.10) walk-forward Sharpe 비교: inverse-vol ΔSharpe **평균 +0.026**(미미), 의미있는 개선(+0.1↑) **1/15**(bio만), 악화(−0.1↓) 0/15. ★us 대형주는 오히려 −0.02~−0.03(vol-가중이 분산 효과 < 집중 손해). → inverse-vol 개선 불충분=채택 안 함, capped-EW 유지(자문 Q4 "capped-EW가 MVO보다 OOS 우위" 정합, inverse-vol 중간지점도 무익). 강도틸트는 측정 안 함(plan ★반대=밸류트랩+과적합).
