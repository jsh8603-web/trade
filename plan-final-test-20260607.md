...
- [ ] **P7-1** 분기별 손익 시계열(port_seq)에서 손실 최대 분기 top-N 식별(실측, drawdown/분기수익 기준).
- [ ] **P7-2** 각 손실 분기의 **자산군×regime×업종(×종목)** 손실 성분분해 — 어느 자산/국면이 끌어내렸나(★**18축 전체로 분해** + 분기 슬라이스, ⑱ yaml↔런타임 배선 누락 포함 = 손실 분기가 "스터디는 됐는데 런타임 미배선" 탓인지 검출).
- [ ] **P7-3** 원인 L0~L3 귀속 → 수정/검토 분기 발생 시 ★위 처리 규칙대로 (즉시 plan/progress 반영·복잡=자문3R·단순=즉시수정·MANDATE 동기화).
