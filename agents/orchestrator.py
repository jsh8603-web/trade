...
외부 데이터는 뉴스랑(NewsRang) 에이전트가 11개 소스에서 병렬 수집하여 제공한다.
...
class Orchestrator:
    ...
    def _decide_target(self, current: str, ms: dict,
        ...
        # ══════════════════════════════════════════
        # 기회 소멸 시 공격적 하향 (v8.2: 2026-04-20)
        # ══════════════════════════════════════════

        # 공격적 에이전트가 기회 점수 낮고 + 하락 추세면 보통으로 하향
        # 근거: 4/18~19 사례 — danger 낮아 전환 안 되고 하락장에서 연속 매수 손실
        if current == "aggressive" and opportunity < 30 and ms["price_change_24h"] < -1:
            self._switch_reason = (
                f"기회 소멸 하향 (기회 {opportunity}점 < 30, "
                f"24h {ms['price_change_24h']:+.1f}%) "
                f"-- 하락 추세에서 공격적 유지 불합리 → 보통"
            )
            return "moderate"
