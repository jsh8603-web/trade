...
- **S2 mediator wiring** — study room state-space posterior **filtered-only** 구독(versioned, covariance/entropy 노출). capital-at-risk 미증가.
  - ★**macro 구독측 ✅ 완료**(2026-06-06, `model: opus` direct): `core/assume/macro_mediator.py`. MacroView(regime_now PIT filtered)→`belief_from_macro_view`(기존 brain 어댑터 재사용)→4-state belief→target 국면 질량 op threshold→`posterior_holds`(bool)→`eval_mediator`. fail-closed UNKNOWN(None/unavailable/stale-too-old/라벨오타), entropy 게이트(평탄 국면 보류), filtered-only(regime_now만·forecast 무시=look-ahead 차단). fhc/belief adapter 무수정 INV-11 byte-identical, 81 passed. observable_ref="regime:{BLOC}:{Label}". ★full posterior 벡터/entropy 원본 미노출이나 belief 변환 레이어가 분포 구성→어댑터서 entropy 산출(caveat 해소). covariance 노출=후속(현 mediator 계약은 boolean holds라 미요구).
  - ⏸ **RegimeGlasso 공급측**(sleeve cov, button INV-9 breaker, .coord §6 미해결)=button 합류 선결. `→ button 세션`
...
- **S5 리서치 통합** — 2-stage(소형 LLM 요약 → 임베딩 인덱싱 RAG → 대형 LLM 카드발권, PIT 스탬프). **발권 단위 = 거시/섹터/종목 scope**(개별 종목 소식 트리거 + universe 정책 = S6 §종목 트리거 준수). VaultVoice BGE/LanceDB 재사용. `wf: harness2`
...
- **S6 [button 세션]** 종목 바스켓 — 룰코어 top-N×지표편차 스크린 + 딥모델 비교선택 + 메타카드. KIS equity. `→ button`
  - ★**종목 단위 카드 발권 트리거 + universe 정책(2026-06-06 사용자 확정 = 단계적)**:
    - **발권 트리거**: 개별 종목 리포트·공시·기사의 **새 소식 = 종목 카드 발권/갱신 트리거**. 단 시계 rate-cap(상한) AND 정보델타 게이트(임베딩 novelty + 수치 리비전, S5 빈도설계 준용) 통과 시에만 → 무의미·중복 소식은 발권 안 함.
    - **universe 정책 = 단계적**: ①**기본(초기 구축·검증) = 스크린 통과분 내 한정**(C6 유지) — 룰코어가 추린 바스켓 내 종목의 소식만 그 종목 카드 근거. ②**universe 밖 신선 종목** = 소식 트리거 시 **즉시 끌어오기 X → 임시 후보 풀에 적재 → 다음 스크린 주기 또는 rate-cap 슬롯에서만 승격**(발권 폭증·PIT 검증부하 방지). ③안정·검증 후 임시 승격을 별도 게이트(rate-cap + 직교성 발권게이트 S4)와 함께 허용.
    - ⛔ "임의 종목에 소식=즉시 카드"(빈도 상한 소실) 금지 — C6 universe 제약 = 의도된 안전장치.
