"""Fetch missing FRED leading-variable candidates for eq_us_cyclical merit study.
Candidates (ISM-free alternatives): PERMIT, T10Y3M, ACOGNO (+ NEWORDER/AMTMNO/DGORDER/T10Y2Y/CFNAI cached).
Copper-gold ratio via FRED (PCOPPUSDM monthly) and yfinance (HG=F/GC=F) for cross-check.
Also probes ALFRED first-release vintage to assess revised-vs-live PIT for monthly macro.
...
def fetch_series(series_id):
    """Fetch full FRED series, save CSV in same schema as cached files."""
...
def probe_alfred_first_release(series_id, n_check=6):
    """For a monthly macro series, fetch ALFRED vintage and report publication lag
    (gap between observation period and first realtime_start) to gauge live-vs-revised PIT.
    """
    url = f'{BASE}/series/observations'
    params = {'series_id': series_id, 'api_key': KEY, 'file_type': 'json',
              'realtime_start': '1990-01-01', 'realtime_end': '9999-12-31',
              'output_type': '4'}  # output_type 4 = initial release + all vintages
    try:
        r = requests.get(url, params=params, timeout=30)
        r.raise_for_status()
        obs = r.json()['observations']
    ...
    # first release = earliest realtime_start per observation date
...
if __name__ == '__main__':
    print('=== Fetch missing FRED candidates ===')
    new_series = ['PERMIT', 'T10Y3M', 'ACOGNO', 'PCOPPUSDM']
    ...
    print('\n=== Probe ALFRED first-release pub lag (PIT awareness) ===')
    pit = {}
    for sid in ['NEWORDER', 'AMTMNO', 'DGORDER', 'ACOGNO', 'PERMIT', 'CFNAI']:
        info = probe_alfred_first_release(sid)
        pit[sid] = info
        if 'error' in info:
            print(f'  {sid}: ALFRED probe error: {info["error"]}')
        else:
            print(f'  {sid}: median pub lag {info["median_pub_lag_days"]}d, max {info["max_pub_lag_days"]}d')
        time.sleep(0.3)
    with open(ROOT / 'merit-pit-probe.json', 'w') as f:
        json.dump(pit, f, indent=2)
    print(f'\nSaved PIT probe -> {ROOT / "merit-pit-probe.json"}')
