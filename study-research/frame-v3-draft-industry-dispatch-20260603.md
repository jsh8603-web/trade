...
- ★**산업별 (a)유효 신호 (b)부호 (c)valuation 적합도 (d)risk factor (e)데이터 양식 전부 상이 = 동적가중**(regime/산업 conditional IC, `derive_weights w∝Θ_signal·IC`) **정당화 데이터 지지** = 사용자 "지표 상관 계속 변하는 그림" 방향 실증. 7산업이 momentum/reversal/regime-conditional/valuation(×3)/변동성 각 대표 = **4 archetype 커버**(cyclical/spread_driven/asset_stable/event_driven). ★단 신호 강도 차등 = battery momentum 만 CONFIRMED(BY 생존), 나머지 PARTIAL/TENTATIVE(BY 미생존·small-n) = "전수 입증" 아닌 "방향 커버"(consumer·us_cyclical reversal TENTATIVE 반영).
...
- ★**peak-EPS trap 메커니즘 정정**: 1차 원인 = 적자 아니라 **earnings cycle 진폭**(흑자 기업도 peak EPS 高→저PER trap 작동). 적자비율은 2차 증폭기. ⛔**시총에 인과 귀속 금지**(size factor 혼동 — 시총은 earnings 안정성 proxy일 뿐). PER → **E/P(earnings yield, 음수익 연속 처리)** 또는 Gross Profitability/PCFR 대체 권고. 미국 us_cyclical PER○ = ★**방향 약 prior**(BY 생존 0 + 24M degenerate strip 후 유의 horizon=3M 1개[BY 미생존] = ★PARTIAL→TENTATIVE 강등, Phase 1 audit). "consistent with"(동일 파이프라인 재측정 아님, ⛔"explained by" 아님) + ★**전이 불가 특이점**(한국 부호반대 = cross-market 일반성 약화, 별도 판정 — round-N.md). ★**peak-EPS = 시장·시총 의존**(한국 중소형 적자多 PER✗ vs 미국 대형 흑자 PER○ 방향) = 동적가중·metric 적합도 시변 증거(미국 PER 약 prior라 일반성 잠정).
