...
Path("exposure-card-v3.json").write_text(json.dumps(
    {"as_of": str(pxm.index[-1].date()), "n_universe": len(card_out),
     "standardization": "sector-neutral cross-sectional z = (x - battery_universe_mean)/std (local, R2-1 gap 메움)",
     "note": "valuation(PBR/PER) z-score = pykrx 스냅샷 부재로 미산출 → 가격신호 z만. supervisor 조립(score→target weight)은 dispatch 밖.",
