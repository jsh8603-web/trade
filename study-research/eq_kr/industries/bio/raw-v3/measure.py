...
def lit_regime_g1(signals, pxm):
    """★bio = XBI(biotech ETF) yoy regime (G1 ex-ante). biotech sentiment/유동성 사이클 proxy.
    event_driven: 임상 binary = 횡단면 측정 어려움. XBI bull/bear = biotech risk-appetite regime."""
    try:
        import yfinance as yf
        xbi = yf.download("XBI", start="2019-01-01", end="2026-05-29", progress=False, auto_adjust=True)["Close"].squeeze()
        xbi.index = pd.to_datetime(xbi.index)
        xbi_m = xbi.resample("ME").last()
        xbi_yoy = xbi_m / xbi_m.shift(12) - 1
        regime = pd.Series(np.where(xbi_yoy > 0.15, "bull", np.where(xbi_yoy < -0.15, "bear", "neutral")), index=xbi_yoy.index)
        out = {"regime_counts": regime.value_counts().to_dict(),
               "note": "XBI(biotech ETF) yoy regime ex-ante. biotech risk-appetite/유동성 사이클. KR bio = XBI 동조.",
        ...
        return dict(available=False, note=f"XBI fetch fail {repr(e)[:80]}")
