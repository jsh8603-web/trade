# -*- coding: utf-8 -*-
"""measure_cross.py — chemical §M.3 cross 축 + PIT 보고지연 stamp. semiconductor 미러 (cycle proxy 교체).
...
§M.3 cross = 3소비자 라우팅 (산업 subagent = 보고만, 조립 안 함):
  ...
  (2) customer-supplier momentum / cycle 선행 (alpha 후보) — 화학 cycle 선행:
      ★화학 = 유가(원재료 납사)·글로벌 화학 cycle upstream. proxy = oil(Brent/WTI, 원재료) +
      global chemical ETF(글로벌 화학 cycle) lagged momentum → 한국 화학 forward.
      directional_spillover 빈 [] 제출 금지(G-A) — 화학 cycle 선행신호 후보 보고 의무.
...
def load_semi_index():
    """chemical universe 동일가중 월간 수익 (산업 대표 시계열). (함수명 semi = 미러 잔존, 화학 universe 읽음)"""
...
def common_factor_beta(semi_ret: pd.Series, factors: pd.DataFrame):
    ...
    df = pd.concat([semi_ret.rename("y"), feat], axis=1).dropna()
    if len(df) < 24:
...
def customer_supplier_momentum(semi_ret: pd.Series):
    """화학 cycle 선행: 유가(원재료 납사 proxy=oil)·글로벌 화학 cycle(global chem ETF) → 한국 화학.
    Cohen-Frazzini: 원재료/선행 cycle lagged 변화 → 다운스트림 forward 예측.
    측정 = oil(WTI)/global chemical ETF lagged 3M momentum → 한국 화학 forward 1M return.
    ★유가-화학 관계 = spread 매개라 부호 모호(theory-notes §1) → lead-lag IC로 실측 판정.
    ★alpha 후보로 검증 대상 (여기선 1차 lead-lag IC 만, null이면 skip 기록 §M.10 결함2 패턴)."""
    import yfinance as yf
    out = {}
    # oil = 원재료(납사) cycle / VAW(글로벌 소재) = 화학 포함 글로벌 cycle / IYM(US 기초소재)
    for sym, label in [("CL=F", "oil_wti_feedstock_cycle"),
                       ("VAW", "global_materials_cycle"),
        ...
        try:
            ...
            for lag in [0, 1, 2, 3]:
                x = sig.shift(lag)
                fwd = semi_ret.shift(-1)  # 한국 화학 다음달 수익
    ...
    out["_note"] = ("oil(WTI 납사 원재료)/global materials(VAW)/US materials(XLB) lagged 3M momentum → 한국 화학 forward 1M. "
                    "유가-화학 = spread 매개 부호 모호. null이면 skip 기록(§M.10 패턴). 동조성분 RegimeGlasso Ω 자동흡수.")
    return out
...
def main():
    semi_ret = load_semi_index()
    print(f"semiconductor industry index: n={len(semi_ret)} months")

    print("[1/4] common factor β ...")
    factors = load_common_factors()
    cfb = common_factor_beta(semi_ret, factors)

    print("[2/4] customer-supplier momentum ...")
    csm = customer_supplier_momentum(semi_ret)
    ...
    print("\n=== customer-supplier momentum (global semi cycle → 한국 반도체 fwd) ===")
