...
def lit_regime_g1(signals, pxm):
    """★financial = 금리 regime (G1 ex-ante). 은행 NIM = 금리 상승 수혜 → regime별 IC 부호 점검.
    금리 = US 10Y(^TNX) Δ regime (KR 금리 proxy, 동조 강). rate_up = NIM 개선 = 은행주 강세 가설.
    ★regime-conditional 부호 점검: momentum/valuation IC 가 rate regime 따라 부호 유지하는지."""
    try:
        ...
        rate_chg = tnx_m.diff(3)  # 3M 금리변화 (ex-ante: 진입시점 관측가능)
        regime = pd.Series(np.where(rate_chg > 0.3, "rate_up",
                                    np.where(rate_chg < -0.3, "rate_down", "rate_flat")), index=rate_chg.index)
        # regime-conditional IC: mom_6 신호 → 12M forward, regime별 부호
        out = {"regime_counts": regime.value_counts().to_dict(),
               "note": "금리(US 10Y) 3M Δ regime ex-ante. 은행 NIM = rate_up 수혜 가설. KR 금리 proxy(동조 강).",
        ...
        return dict(available=False, note=f"rate fetch fail {repr(e)[:80]}")
