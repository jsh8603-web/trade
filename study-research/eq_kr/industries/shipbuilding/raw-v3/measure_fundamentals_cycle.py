# -*- coding: utf-8 -*-
"""measure_fundamentals_cycle.py — 조선 신규 cycle 지표 forward IC 측정 (S2, ★이연 금지). semiconductor 미러.

사용자 박제: 신규 발굴 지표(재고/CAPEX/R&D) 지금 측정 + verdict 박제(tentative라도) + 현 가능 검증 전부.

★★조선 capex 부호 prior 재검토 (team-lead 박제 = capex 일반화 가설의 조선 특수성):
  자동차/반도체 = capex_ratio(유형자산 asset growth) 음 prior(Cooper-Gulen-Schill 2008 과잉투자 anomaly)가
  structural_prior. ★단 조선은 ★orderbook(수주잔고) 기반 장치산업 → 부호 prior 가 시장과 다를 수 있음:
  - 일반론(asset growth anomaly): 유형자산↑ = 과잉투자 → forward 음. (대다수 산업)
  - 조선 특수론: 2010년대 과잉설비 → 2016-2020 구조조정(도크 매각/감축) → 2023~ 수주호황 = 설비 재가동.
    수주 cycle 상승기엔 ppe↑/재가동이 ★수주확대 proxy(양 가능). = 부호 데이터로 판정(one-sided 단정 금지).
  → ★two-sided 표기: capex/ppe = "음 prior(asset growth 일반론) 약 + 양 가능(조선 수주 proxy)". 측정 부호 그대로 보고.

신호 (DART dart_extended.parquet, PIT rcept_dt 이후만):
  - inv_ratio    = 재고자산/총자산 (★조선 재고 = 건조 중 재공품/미인도선 → 수주호황기 재고↑=미래매출 = ★양 가능,
                   반도체 음 prior 와 다름. = two-sided, 데이터 판정)
  - inv_yoy      = 재고자산 yoy (재고 증가율, two-sided)
  - capex_ratio  = 유형자산/총자산 (★조선 = asset growth 음 prior 약 + 수주확대 proxy 양 가능 = two-sided)
  - ppe_yoy      = 유형자산 yoy (CAPEX cycle, ★조선 two-sided = 설비투자가 호황신호 가능)
  - rnd_ratio    = 무형자산/총자산 (R&D proxy, ★양 prior: 기술경쟁력 — LNG선/친환경선 기술)
...
# ★조선 부호 prior 재검토 (team-lead capex 일반화): orderbook 기반이라 asset growth/재고 = two-sided.
#   음 prior = asset growth anomaly 일반론 / 양 가능 = 조선 수주확대 proxy. 측정 부호 그대로 판정.
PRIOR = {"inv_ratio": "two-sided(양가능:재공품)", "inv_yoy": "two-sided", "capex_ratio": "two-sided(음약+양가능:수주proxy)",
         "ppe_yoy": "two-sided(음약+양가능)", "rnd_ratio": "양(친환경선기술)"}
