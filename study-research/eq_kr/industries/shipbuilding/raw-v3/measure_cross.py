# -*- coding: utf-8 -*-
"""measure_cross.py — shipbuilding(조선) §M.3 cross 축 + PIT 보고지연 stamp. semiconductor 미러.
...
§M.3 cross = 3소비자 라우팅 (산업 subagent = 보고만, 조립 안 함):
  ...
  (2) customer-supplier momentum (alpha 후보) — 조선 supply-chain 선행:
      ★조선 = 글로벌 신조선 수요 cycle. proxy = BDRY(벌크 운임, 해운 수요→신조선 발주 선행) + CL=F(유가, 해양플랜트/탱커 수요)
      + 글로벌 해운/조선(BOAT ETF). 운임 상승 → 선주 발주 증가 → 조선 수주 lagged 선행 가설.
...
def customer_supplier_momentum(semi_ret: pd.Series):
    """조선 supply-chain 선행: 글로벌 해운 운임(BDRY) / 유가(CL=F) / 글로벌 조선·해운(BOAT) → 한국 조선.
    Cohen-Frazzini: 선행 cycle(운임·유가) lagged return → 다운스트림(조선 수주→주가) forward 예측.
    가설: 운임 상승 → 선주 발주 증가 → 조선 수주 lagged 선행 / 유가 상승 → 해양플랜트·탱커 수요.
    측정 = proxy lagged 3M momentum → 한국 조선 forward 1M return.
    ★alpha 후보로 검증 대상 (여기선 1차 lead-lag IC 만)."""
    import yfinance as yf
    out = {}
    for sym, label in [("BDRY", "bdry_dry_bulk_freight"), ("CL=F", "oil_offshore_tanker_demand"),
                       ("BOAT", "boat_global_shipping_shipbuild")]:
        try:
            s = yf.download(sym, start=START, end=END, progress=False, auto_adjust=True)["Close"].squeeze()
            ...
            for lag in [0, 1, 2, 3]:
                x = sig.shift(lag)
                fwd = semi_ret.shift(-1)  # 한국 조선 다음달 수익
    ...
    out["_note"] = ("글로벌 운임(BDRY)/유가(CL=F)/글로벌조선해운(BOAT) lagged 3M momentum → 한국 조선 forward 1M. "
                    "alpha 후보 = 측정 후 IC→weight. 동조성분은 RegimeGlasso Ω 자동흡수(이중라우팅 X). "
                    "★조선 신조선 수주(Clarkson)/SCFI 운임 직접지표 = 유료 → ETF proxy(collector_plan).")
    return out
