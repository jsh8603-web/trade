# -*- coding: utf-8 -*-
"""measure_valuation.py — shipbuilding(조선) valuation 횡단면 IC (§M.7 핵심). semiconductor 미러.

DART 재무 + pykrx 가격 → PBR/PER cross-sectional z → forward IC.

★조선 archetype = cyclical (orderbook 기반 수주 cycle), primary_metric = ev_ebitda (peak-EPS 경계):
  - 조선 = 수주잔고(orderbook) → 매출 인식 시차 大. 2019-22 장기 적자(저가수주 후유증) → PER 무의미 구간 多,
    2023~ 흑자전환(고선가 수주 인식) → 적자 지배로 PER cross-sectional 부적합 위험(bio 적자 동형). PBR 더 적합 prior.
  - 본 측정 = PBR/PER cross-sectional (EV-EBITDA 는 부채/현금 추가 계정 = collector_plan).
...
PIT 엄수:
  ...
  - 시총_t = Close_t × shares (★현재 주식수 근사 — 시점별 주식수 = collector_plan).
  - PBR = 시총 / 자본총계(equity, BS). PER = 시총 / TTM 순이익.

측정:
  - cross-sectional z-score(sector-neutral) → valuation cs_rank_IC(forward).
...
def ttm_net_income(ni_df):
    """TTM 순이익: 최신 FY 우선 (가장 안정), 없으면 분기 누적 annualize."""
    if len(ni_df) == 0:
        return None
...
def main():
    ...
    out = {"meta": {"universe_n": int(px.shape[1]),
                    ...
                    "archetype_note": "★조선 cyclical(orderbook): 2019-22 적자→2023~ 흑자전환. 적자 지배로 PER cross-sectional 부적합 위험(bio 동형). PBR 더 적합 prior.",
