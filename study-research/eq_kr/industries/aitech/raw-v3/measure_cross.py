# -*- coding: utf-8 -*-
"""measure_cross.py — semiconductor §M.3 cross 축 + PIT 보고지연 stamp. battery 미러.
...
§M.3 cross = 3소비자 라우팅 (산업 subagent = 보고만, 조립 안 함):
  ...
  (2) customer-supplier momentum (alpha 후보) — 반도체 supply-chain 선행:
      ★반도체 = 장비/소재 upstream lag. proxy = SOXX(글로벌 반도체 cycle) + NVDA(AI/HBM 수요).
      반도체는 upstream(장비/소재) → downstream(메모리/파운드리) 동조라, 글로벌 cycle proxy lagged momentum 측정.
...
def load_semi_index():
    """semiconductor universe 동일가중 월간 수익 (산업 대표 시계열)."""
...
def customer_supplier_momentum(semi_ret: pd.Series):
    """★AItech 글로벌 driver 선행 (semiconductor SOXX/NVDA → aitech proxy 교체):
    - QQQ (NASDAQ100) = 글로벌 인터넷/AI/SW cycle proxy (메타/구글 광고 + MS/아마존 클라우드 + 게임).
    - NVDA = AI capex/데이터센터 수요 sentiment (AI SW·플랫폼 수혜 leading proxy, hyperscaler capex 대리).
    Cohen-Frazzini: 글로벌 tech cycle lagged momentum → 한국 AItech forward 예측.
    측정 = QQQ/NVDA lagged 3M momentum → 한국 aitech forward 1M return.
    ★alpha 후보로 검증 대상 (여기선 1차 lead-lag IC 만). null 이면 §D forward-alpha falsifier 정상 작동."""
    import yfinance as yf
    out = {}
    for sym, label in [("QQQ", "qqq_global_tech_cycle"), ("NVDA", "nvda_ai_capex_demand")]:
        try:
            s = yf.download(sym, start=START, end=END, progress=False, auto_adjust=True)["Close"].squeeze()
            s.index = pd.to_datetime(s.index)
            sm_ = s.resample("ME").last()
            sig = sm_.pct_change(3)  # 3M leading momentum
            res = {}
            for lag in [0, 1, 2, 3]:
                x = sig.shift(lag)
                fwd = semi_ret.shift(-1)  # 한국 aitech 다음달 수익
    ...
    out["_note"] = ("global tech cycle(QQQ)/AI capex demand(NVDA) lagged 3M momentum → 한국 AItech forward 1M. "
                    "alpha 후보 = 검증 후 IC→weight. 동조성분은 RegimeGlasso Ω 자동흡수(이중라우팅 X).")
    return out
