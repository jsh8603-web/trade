...
from measure_conditional import (
    load, build_price_signals, build_valuation_signals, forward_returns_daily, ic_series, measure_interaction_terms,
)
...
def main():
    ...
    val_sigs = build_valuation_signals(px, fin, uni)   # ★aitech: valuation 이 primary (pbr/per BY 생존)
    ...
    results = {"split": {"IS": IS, "OOS": OOS},
               "note": "aitech: ①momentum reversal(음, growth 가설 반증=battery 반대) ②★valuation pbr/per(BY 생존=primary tradeable) ③flow_strong_buy interaction 음 증폭 의 walk-forward OOS 검정.",
               "unconditional": {}, "valuation": {}, "regime_cell": {}, "interaction": {}}

    # 1) unconditional momentum (y_60d 본진 + y_20d 메인) — reversal 부호 OOS 유지?
    ...
    # 1.5) ★valuation walk-forward (aitech primary = pbr/per BY 생존) — y_60d 본진(긴 horizon value)
    for sig in ["pbr_z", "per_z"]:
        for h, hd in [("y_60d", 60), ("y_20d", 20)]:
            fwd = forward_returns_daily(px, anchor, hd)
            ic = ic_series(val_sigs[sig], fwd)
            sp = split_ic(ic)
            if sp:
                results["valuation"][f"{sig}__{h}"] = sp

    # 2) regime cell: flow_strong_buy (★aitech reversal 증폭축) — IS/OOS
    flow_sb_months = lab19["flow_regime"][lab19["flow_regime"] == "flow_strong_buy"].index
    for sig in ["mom_12_1", "vol_60", "pbr_z", "per_z"]:
        fwd = forward_returns_daily(px, anchor, 20)
        ic = ic_series(all_sigs[sig], fwd)
        ic_fsb = ic[ic.index.isin(flow_sb_months)]
        sp = split_ic(ic_fsb)
        if sp:
            results["regime_cell"][f"{sig}__y20d__flow_strong_buy"] = sp

    # 2.5) ★per_z KRW_weak (conditional 최강 cell wc_p=0.0025) walk-forward
    krw_weak_months = lab19["krw_regime"][lab19["krw_regime"] == "KRW_weak"].index
    for sig in ["per_z", "pbr_z"]:
        fwd = forward_returns_daily(px, anchor, 60)
        ic = ic_series(all_sigs[sig], fwd)
        ic_kw = ic[ic.index.isin(krw_weak_months)]
        sp = split_ic(ic_kw)
        if sp:
            results["regime_cell"][f"{sig}__y60d__KRW_weak"] = sp

    # 3) flow_strong_buy interaction term IS/OOS (aitech primary conditional = reversal 증폭)
    for sig in ["mom_12_1", "vol_60", "rev_1m"]:
    ...
    print("\n=== ★valuation walk-forward (aitech primary = pbr/per BY 생존) ===")
    for k, v in results["valuation"].items():
        print(f"  {k:18s} IS={v['is_ic']:+.4f}(n={v['is_n']}) OOS={v['oos_ic']:+.4f}(n={v['oos_n']}) {v['verdict']}")
    print("\n=== flow_strong_buy / KRW_weak regime cell walk-forward ===")
