...
def soxx_regime_g1():
    """★QQQ yoy regime (진입시점 관측가능 = ex-ante). AItech 글로벌 cycle proxy.
    semiconductor 의 SOXX yoy 대응 — NASDAQ100(인터넷/AI/SW) 글로벌 tech cycle 라벨(bull/bear/neutral)."""
    try:
        import yfinance as yf
        qqq = yf.download("QQQ", start="2019-01-01", end="2026-05-29", progress=False, auto_adjust=True)["Close"]
        qqq = qqq.squeeze()
        qqq.index = pd.to_datetime(qqq.index)
        qqq_m = qqq.resample("ME").last()
        qqq_yoy = qqq_m / qqq_m.shift(12) - 1
        regime = pd.Series(np.where(qqq_yoy > 0.2, "bull", np.where(qqq_yoy < -0.1, "bear", "neutral")), index=qqq_yoy.index)
        return dict(regime_counts=regime.value_counts().to_dict(),
                    note="QQQ yoy regime ex-ante (진입시점 NASDAQ100 yoy 관측가능). 글로벌 tech/AI cycle proxy (AItech).",
                    available=True)
    except Exception as e:
        return dict(available=False, note=f"QQQ fetch fail {repr(e)[:80]}")
