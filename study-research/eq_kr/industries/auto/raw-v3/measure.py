...
def lit_regime_g1(signals, pxm):
    """★auto = USDKRW(원화) regime (G1 ex-ante). 자동차 = 수출주 = 원화 약세 수혜(cyclical).
    krw_weak(USDKRW yoy↑) = 수출 마진 개선 = 자동차 강세 가설 → regime별 IC 부호 점검."""
    try:
        import FinanceDataReader as fdr
        fx = fdr.DataReader("USD/KRW", "2018-01-01", "2026-05-29")["Close"].rename("USDKRW")
        fx.index = pd.to_datetime(fx.index)
        fx_m = fx.resample("ME").last()
        fx_yoy = fx_m / fx_m.shift(12) - 1
        regime = pd.Series(np.where(fx_yoy > 0.05, "krw_weak",
                                    np.where(fx_yoy < -0.05, "krw_strong", "krw_neutral")), index=fx_yoy.index)
        out = {"regime_counts": regime.value_counts().to_dict(),
               "note": "USDKRW yoy regime ex-ante. 자동차 수출주 = krw_weak(원화 약세) 수출마진 수혜 가설.",
               "available": True, "regime_conditional_ic": {}}
        sig = signals.get("mom_6")
        if sig is not None:
            fwd = pxm.shift(-12) / pxm - 1
            for reg in ["krw_weak", "krw_strong", "krw_neutral"]:
        ...
        return dict(available=False, note=f"USDKRW fetch fail {repr(e)[:80]}")
