# -*- coding: utf-8 -*-
"""exposure_card.py — §M.7 cross-sectional peer-relative z-score exposure card.
universe 각 종목의 현 시점 momentum 지표 횡단면 z-score (sector-neutral 표준화 = subagent 담당).
※ valuation(PBR/PER) z-score = 데이터 부재(pykrx 스냅샷 죽음) → 가격신호 z-score만."""
import sys, io, json
...
Path("exposure-card-v3.json").write_text(json.dumps(
    {"as_of": str(pxm.index[-1].date()), "n_universe": len(card_out),
     "standardization": "sector-neutral cross-sectional z = (x - auto_universe_mean)/std (local, R2-1 gap 메움)",
     "signal_sign_note": "★자동차 momentum forward IC = 약 음(reversal 방향)·비유의(universe 17종 작음). mom_6_z 상위 = forward 약. selection = supervisor 가 IC 부호·valuation 반영.",
