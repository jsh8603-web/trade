...
def main():
    ...
    # ★resume: 기존 parquet 로드 → 완료 code skip (foreground 청크 재실행 = background 미유지 환경 대응)
    DONE = DATA / "dart_done_codes.json"
    if cache.exists():
        prev = pd.read_parquet(cache)
        rows = prev.to_dict("records")
    else:
        rows = []
    done_codes = set(json.loads(DONE.read_text())) if DONE.exists() else set()
    if done_codes:
        print(f"resume: {len(rows)} rows, {len(done_codes)}/{len(codes)} codes done")
        ...
        for year in years:
            for qlabel, rcode in REPRT.items():
                try:
                    ...
                    pass
                time.sleep(0.15)  # rate limit (분당 ~400, DART 1000/min 한도 내)
        # ★incremental save (매 종목) — timeout 돼도 진행분 보존
        done_codes.add(code)
        pd.DataFrame(rows).to_parquet(cache)
        DONE.write_text(json.dumps(sorted(done_codes)))
        print(f"  [{ci+1}/{total}] {code} {names[code]} done, rows={len(rows)}", flush=True)
    df = pd.DataFrame(rows)
    df.to_parquet(cache)
    print(f"\nSaved {cache}: {len(df)} rows", flush=True)
    if len(df):
