# -*- coding: utf-8 -*-
"""collect.py — semiconductor §M v3 universe 확장 + 횡단면 패널 수집.

frame v3 §M.7 exposure card 계약 데이터 수집 단계. battery pilot collect.py 미러
(BATTERY_KW → SEMI_KW, 나머지 floor/PIT 로직 동일).

산출:
- data/universe.parquet        — semiconductor universe (FDR 섹터 매칭, 시총·ADV floor)  [현재 스냅샷 = PIT 멤버십 한계]
...
★데이터 제약 (battery pilot 발견 동일):
  ...
  - FDR KRX-DESC = Sector/Industry/Products 보유 → universe 멤버십 구성 (현재 스냅샷, PIT 멤버십 아님 = 생존편향 한계).

★반도체 sub-cluster (round-1.md): 메모리(삼성전자/SK하이닉스) / 파운드리(DB하이텍 등) / 소부장(장비·소재·부품).
  KOSPI 집중(005930+000660 시총 50%+) → cap-weighted IC 로 microcap 지배 점검 의무.
"""
...
# 시총·유동성 floor (방법론 방향, CPCV 캘리브레이션 대상 — §M.6). battery 와 동일 floor.
MARCAP_FLOOR = 3e11   # 3000억원
ADV_FLOOR = 3e9       # 30억원/일 (listing Amount 스냅샷)

# 반도체 키워드: 메모리/파운드리/소부장(장비·소재·부품) 포괄.
# ⚠️ "반도체" 단독은 장비·소재까지 폭넓게 매칭 → sub-cluster 통합 universe (round-1 §1).
SEMI_KW = ["반도체", "메모리", "DRAM", "파운드리", "웨이퍼", "포토마스크",
           "반도체장비", "반도체소재", "반도체부품", "후공정", "전공정",
           "패키징", "테스트", "포토레지스트", "식각", "증착"]


def build_universe() -> pd.DataFrame:
    """FDR 섹터/산업/제품 키워드 → semiconductor universe + 시총·ADV floor."""
    ...
    semi = m[mask].copy().sort_values("Marcap", ascending=False)
    semi["pass_floor"] = (semi["Marcap"] >= MARCAP_FLOOR) & (semi["Amount"] >= ADV_FLOOR)
    semi.to_parquet(cache)
    return semi
