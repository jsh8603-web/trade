...
def customer_supplier_momentum(semi_ret: pd.Series):
    """반도체 supply-chain 선행: 글로벌 반도체 cycle(SOXX) / AI 수요(NVDA) → 한국 반도체.
    Cohen-Frazzini: 업스트림/선행 cycle lagged return → 다운스트림 forward 예측.
    측정 = SOXX/NVDA lagged 3M momentum → 한국 반도체 forward 1M return.
    ★alpha 후보로 17종 검증 대상 (여기선 1차 lead-lag IC 만)."""
    import yfinance as yf
    out = {}
    for sym, label in [("SOXX", "soxx_global_semi_cycle"), ("NVDA", "nvda_ai_hbm_demand")]:
        try:
            s = yf.download(sym, start=START, end=END, progress=False, auto_adjust=True)["Close"].squeeze()
            ...
            for lag in [0, 1, 2, 3]:
                x = sig.shift(lag)
                fwd = semi_ret.shift(-1)  # 한국 반도체 다음달 수익
    ...
    out["_note"] = ("global semi cycle(SOXX)/AI demand(NVDA) lagged 3M momentum → 한국 반도체 forward 1M. "
