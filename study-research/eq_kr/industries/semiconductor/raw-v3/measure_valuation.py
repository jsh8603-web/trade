# -*- coding: utf-8 -*-
"""measure_valuation.py — semiconductor valuation 횡단면 IC (§M.7 핵심). battery 미러.

DART 재무 + pykrx 가격 → PBR/PER cross-sectional z → forward IC.

★반도체 archetype = cyclical, primary_metric = ev_ebitda (peak-EPS 경계):
  - 사이클 정점에 EPS 높아 PER 낮게 보임(value trap) → PER value premium 부호 반전 위험 주의.
...
def main():
    ...
    out = {"meta": {"universe_n": int(px.shape[1]),
                    ...
                    "archetype_note": "★반도체 cyclical: 사이클 정점 EPS 高→PER 低(value trap). PER value premium 부호 반전 위험 주의.",
