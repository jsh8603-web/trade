# -*- coding: utf-8 -*-
"""measure.py — steel §M v3 측정 17종 핵심 실행. semiconductor measure.py 미러.
...
측정 (가격기반 횡단면 신호 — valuation 횡단면은 measure_valuation.py 에서 DART 재구성):
  ...
  ③ regime-conditional 4게이트:
     G1 ex-ante (SLX yoy regime, 진입시점 관측가능 — 글로벌 철강 cycle proxy)
  ...
  ④ 유동성-티어 IC (size-bucket별 IC + cap-weighted IC) — ★철강 POSCO 집중 → cap-weighted 점검 핵심
...
★G1 regime = SLX yoy (VanEck Steel ETF = 글로벌 철강 cycle proxy). 반도체 SOXX 대응.
★철강 universe 협소(23종) = small-breadth → cross-sectional IC magnitude 과대 위험(frame §M.12 haircut).
"""
...
def liquidity_tier_ic(sig: pd.DataFrame, fwd: pd.DataFrame, amt: pd.DataFrame):
    ...
    Rank-IC 동일가중이 microcap 지배하는지 점검. ★철강 POSCO 집중 → 중요."""
...
def main():
    ...
    results["regime_g1"] = slx_regime_g1()
...
def slx_regime_g1():
    """SLX yoy regime (진입시점 관측가능 = ex-ante). 글로벌 철강 cycle proxy.
    VanEck Steel ETF = 글로벌 철강 산업 cycle 라벨(bull/bear/neutral). 반도체 SOXX 대응."""
    try:
        import yfinance as yf
        slx = yf.download("SLX", start="2019-01-01", end="2026-05-29", progress=False, auto_adjust=True)["Close"]
        slx = slx.squeeze()
        slx.index = pd.to_datetime(slx.index)
        slx_m = slx.resample("ME").last()
        slx_yoy = slx_m / slx_m.shift(12) - 1
        regime = pd.Series(np.where(slx_yoy > 0.2, "bull", np.where(slx_yoy < -0.2, "bear", "neutral")), index=slx_yoy.index)
        return dict(regime_counts=regime.value_counts().to_dict(),
                    note="SLX yoy regime ex-ante (진입시점 철강 ETF yoy 관측가능). 글로벌 철강 cycle proxy.",
                    available=True)
    except Exception as e:
        return dict(available=False, note=f"SLX fetch fail {repr(e)[:80]}")
