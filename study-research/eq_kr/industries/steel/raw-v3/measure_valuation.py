# -*- coding: utf-8 -*-
"""measure_valuation.py — steel valuation 횡단면 IC (§M.7 핵심). semiconductor 미러.

DART 재무 + pykrx 가격 → PBR/PER cross-sectional z → forward IC.

★철강 archetype = cyclical, primary_metric = pbr (peak/trough-EPS 왜곡으로 PER 무효 prior):
  - 사이클 정점 EPS 高→PER 低(peak trap) + 다운사이클 적자→PER 무의미(trough) = 양방향 왜곡.
  - 본 측정 = PBR/PER cross-sectional. ★철강 = 구조적 저PBR(P/B<1 多) → cross-sectional 분산 작을 수 有.
...
def main():
    ...
    out = {"meta": {"universe_n": int(px.shape[1]),
                    ...
                    "archetype_note": "★철강 cyclical: 정점 peak-EPS + 다운 trough → PER 양방향 왜곡. PBR(자본 stock) robust prior. ★철강 구조적 저PBR.",
