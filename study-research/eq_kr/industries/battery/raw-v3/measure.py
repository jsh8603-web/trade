...
def lit_regime_g1(signals, pxm):
    """LIT yoy regime (진입시점 관측가능 = ex-ante). 양극재 신호 regime-conditional."""
    try:
        import yfinance as yf
        lit = yf.download("LIT", start="2019-01-01", end="2026-05-29", progress=False, auto_adjust=True)["Close"]
        lit = lit.squeeze()
        lit.index = pd.to_datetime(lit.index)
        lit_m = lit.resample("ME").last()
        lit_yoy = lit_m / lit_m.shift(12) - 1
        regime = pd.Series(np.where(lit_yoy > 0.2, "bull", np.where(lit_yoy < -0.2, "bear", "neutral")), index=lit_yoy.index)
        return dict(regime_counts=regime.value_counts().to_dict(),
                    note="LIT yoy regime ex-ante (진입시점 lithium ETF yoy 관측가능). 양극재 cost-pass-through proxy.",
                    available=True)
    except Exception as e:
        return dict(available=False, note=f"LIT fetch fail {repr(e)[:80]}")
