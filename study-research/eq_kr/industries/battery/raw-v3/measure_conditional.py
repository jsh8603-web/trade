# -*- coding: utf-8 -*-
"""measure_conditional.py — semiconductor conditional IC surface (S2 신규, dispatch 원의도 본체).
...
def main():
    ...
    # ★battery 특화: sign-flip 가장 강한 축 = flow_regime(momentum 양→음 flip, 반도체의 KRW_weak 과 다름).
    #   → flow_strong_buy(passive 지배) interaction 을 primary 로 측정. KRW_weak 도 cross-sector 비교용 병행.
    results["family_2_interaction"] = measure_interaction_terms(
        price_sigs, px, anchor, lab19, dummy_regime=("flow_regime", "flow_strong_buy"))
