# -*- coding: utf-8 -*-
"""measure.py — battery §M v3 측정 17종 핵심 실행.

frame v3 §M.1~M.2 측정축 + §M.7 cross-sectional peer-relative z-score exposure card.

측정 (가격기반 횡단면 신호 — valuation 횡단면은 데이터 부재로 격하, collector_plan 등록):
  ① cross-sectional peer-relative z-score (momentum 12-1, vol, reversal 1M)
  ② forward 횡단면 Rank-IC (horizon-tagged: 1M PEAD / 3M·6M·12M 모멘텀)
  ③ regime-conditional 4게이트:
     G1 ex-ante (LIT yoy regime, 진입시점 관측가능)
  ...
  ④ 유동성-티어 IC (size-bucket별 IC + cap-weighted IC)
  ⑤ leave-episode / within-period / e-process
  ⑥ PIT-fundamentals safety (보고지연 — DART rcept_dt 우회 측정)
  ⑦ 생존편향 (상장 시점 차이 = PIT universe 멤버십 결손 정량화)
  ⑧ net-cost (KR STT sell-side 0.18-0.23% 비대칭 + sqrt impact)

★데이터 제약 정직 반영: PBR/PER 횡단면 스냅샷 부재 → valuation z-score INSUFFICIENT, 가격신호만 측정.
"""
...
def build_signals(px: pd.DataFrame):
    ...
    # 12-1 모멘텀 (t-12M ~ t-1M, 최근월 skip = reversal 제거)
...
def ic_summary(ic: pd.Series, nw_lags: int) -> dict:
    ...
    # block bootstrap CI (block=3M, B=2000)
...
def cpcv_oos_ic(sig: pd.DataFrame, fwd: pd.DataFrame, h_months: int, n_splits=6, n_test=2, embargo=1):
    ...
    for test_combo in combinations(range(n_splits), n_test):
        ...
        # OOS IC = test month IC 평균 (purge/embargo 는 train 영향, IC 자체는 test month 값)
...
def liquidity_tier_ic(sig: pd.DataFrame, fwd: pd.DataFrame, amt: pd.DataFrame):
    ...
    Rank-IC 동일가중이 microcap 지배하는지 점검."""
    ...
    for dt in idx:
        ...
        # cap-weighted IC: ADV 가중 weighted Spearman approx (weighted Pearson on ranks)
...
def main():
    ...
    results = {"meta": {"universe_n": int(px.shape[1]),
                        ...
                        "data_note": "valuation 횡단면(PBR/PER) = pykrx 스냅샷 부재로 INSUFFICIENT. 가격기반 횡단면 신호만 측정."},
               "survivor_bias": survivor}

    # 다중검정 풀: 4 signal × 4 horizon = 16 테스트
    pvals = {}
    indicators = {}
    for sname, sig in signals.items():
        for hname, h in horizons.items():
            ...
            # e-process: IC 시계열 붕괴 (baseline = 0)
    ...
    # G2: FDR (Benjamini-Yekutieli, 신호 상관 → BY 보정)
    ...
    # LIT yoy regime (G1 ex-ante) — 기존 macro 재사용
    results["regime_g1"] = lit_regime_g1(signals, pxm)
...
def lit_regime_g1(signals, pxm):
    """★consumer = VIX risk regime (G1 ex-ante). 방어주(asset_stable) = risk-off 상대 수혜 가설.
    VIX level regime: high(risk-off) = 방어 소비재 강세 → regime별 valuation/momentum IC 부호 점검."""
    try:
        import yfinance as yf
        vix = yf.download("^VIX", start="2019-01-01", end="2026-05-29", progress=False, auto_adjust=True)["Close"].squeeze()
        vix.index = pd.to_datetime(vix.index)
        vix_m = vix.resample("ME").last()
        # VIX level regime (ex-ante: 진입시점 관측). 상위/하위 tercile
        hi, lo = vix_m.quantile(0.67), vix_m.quantile(0.33)
        regime = pd.Series(np.where(vix_m > hi, "risk_off", np.where(vix_m < lo, "risk_on", "neutral")), index=vix_m.index)
        out = {"regime_counts": regime.value_counts().to_dict(),
               "note": "VIX level regime ex-ante. 방어 소비재(asset_stable) = risk_off 상대수혜 가설.",
               "available": True, "regime_conditional_ic": {}}
        # regime-conditional: valuation(pbr/per 대용 = mom_6 placeholder, valuation 은 measure_valuation 에서)
        sig = signals.get("mom_6")
        if sig is not None:
            fwd = pxm.shift(-12) / pxm - 1
            for reg in ["risk_off", "risk_on", "neutral"]:
                reg_months = regime[regime == reg].index
                ics = []
                for dt in sig.index:
                    if dt not in reg_months:
                        continue
                    sv = sig.loc[dt].dropna()
                    rv = fwd.loc[dt].dropna() if dt in fwd.index else pd.Series(dtype=float)
                    common = sv.index.intersection(rv.index)
                    if len(common) >= 5:
                        rho, _ = stats.spearmanr(sv.loc[common], rv.loc[common])
                        if not np.isnan(rho):
                            ics.append(rho)
                out["regime_conditional_ic"][reg] = dict(
                    mom_6_ic=round(float(np.mean(ics)), 4) if ics else None, n=len(ics))
        return out
    except Exception as e:
        return dict(available=False, note=f"VIX fetch fail {repr(e)[:80]}")
