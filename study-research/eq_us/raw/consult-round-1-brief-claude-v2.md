## ⚠️ DIRECT ANSWER MODE — 본 자문 운영 지시 (가장 먼저 읽기)

- 본 요청 = 메모리 기반 직접 답. **reference 정확도 verify / web search / 환각 검증 thinking 일체 금지** (지금 환경 = web search 불가 fresh chat).
- 환각 우려 reference 는 그냥 `tentative reference (need verification)` 라벨 부착 + **답변 본문 자체는 계속 진행**. reference verify 시도하다 응답 끊지 마라.
- 응답 형식 = 두괄식 + 표 + 5 필드 (답변 / 근거 / 정량 magnitude / 반증조건 / confound) 그대로. **Q1-Q10 모두 답해라**. 도중 끊지 마라.
- 응답 길이 = 각 Q 200-500 단어 (총 3000-5000 단어). 짧아도 좋으나 Q1-Q10 모두 cover 의무.
...
eq_us_defensive 합성 의심 (synthetic 240m, seed 20260530). 5 archetype 권고 = staples / utility / healthcare / bank / insurance. real rate 상승 시 utility 음 / bank 양 (부호 반대 sub-sleeve).
...
- 근거 (학술 [저자 연도 DOI/URL] + 실무 [리포트/정책 발행자 연도] + 1차 데이터 [FRED ID / SEC form / OSS lib]) — 환각 우려 시 'tentative' 라벨
